(function (root) {
  ...
  function characterMapContains(code) {
    for (var i = 0; i < charsMap.length; i++) {
      if (charsMap[i][0] === code) return true;
    }
    return false;
  }
  ...
  function getCharRep(code) {
    for (var i = 0; i < charsMap.length; i++) {
      if (charsMap[i][0] === code) return charsMap[i];
    }
    return false;
  }
  ...
  function isTransparent(code) {
    for (var i = 0; i < transChars.length; i++) {
      if (transChars[i] === code) return true;
    }
    return false;
  }
  ...
  root.ArabicReshaper = {
    convertArabicCharacters: convertArabic
  };
