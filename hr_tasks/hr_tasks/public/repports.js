function getReportsUser(win) {
    return (win && win.currentUser) || (typeof currentUser !== 'undefined' ? currentUser : null) || null;
}
...
async function initializeReports(reportsWindow) {
    ...
    function resolveCurrentUser() {
        try {
            // 1) Source d'authentification principale
            if (typeof authManager !== 'undefined' && authManager.isAuthenticated()) {
                const authUser = authManager.getUser() || {};
                const role = authManager.getUserRole();
                const normalized = { ...authUser };
                if (role && !normalized.role) normalized.role = role;
                if (normalized.employeeId && !normalized.employee_id) normalized.employee_id = normalized.employeeId;
                if (!normalized.id) {
                    normalized.id = normalized.user_id || normalized.employee_id || normalized.employeeId || normalized.id;
                }
                return normalized;
            }
        } catch (e) {
            console.warn('Auth-based user resolution failed, falling back', e);
        }

        // 2) Utiliser l'utilisateur déjà présent dans la page parente si disponible
        if (reportsWindow.currentUser) {
            const u = { ...reportsWindow.currentUser };
            if (u.employeeId && !u.employee_id) u.employee_id = u.employeeId;
            if (!u.id) u.id = u.user_id || u.employee_id || u.employeeId || u.id;
            return u;
        }

        // 3) Mode dev: lecture éventuelle depuis l'URL uniquement si dev=1
        try {
    ...
    try {
        ...
        // Déterminer la bonne base backend (éviter l'origine du serveur statique 55xx)
        let backendUrl = null;
        try {
            backendUrl =
                (typeof window !== 'undefined' && window.API_BASE) ||
                (typeof window !== 'undefined' && window.API_SERVICES && window.API_SERVICES.hr_tasks) ||
                (typeof window !== 'undefined' && window.parent && window.parent.API_SERVICES && window.parent.API_SERVICES.hr_tasks) ||
                null;
        ...
        let response = await fetch(`${backendUrl}/api/reports/data`, { cache: 'no-store' });
        if (response.status === 304) {
            // Force a re-fetch if a proxy or browser answered with Not Modified
            response = await fetch(`${backendUrl}/api/reports/data`, { cache: 'reload' });
        }
        ...
        if (data.success) {
            ...
            reportsWindow.currentUser = resolveCurrentUser();
            // S'assurer que currentUser a un id en mode dev (param URL employee_id/user_id)
            if (!reportsWindow.currentUser && typeof currentUser !== 'undefined' && currentUser) {
                // Fallback à l'utilisateur global déjà injecté
                const fallbackUser = { ...currentUser };
                if (fallbackUser.employeeId && !fallbackUser.employee_id) fallbackUser.employee_id = fallbackUser.employeeId;
                if (!fallbackUser.id) fallbackUser.id = fallbackUser.user_id || fallbackUser.employee_id || fallbackUser.employeeId;
                reportsWindow.currentUser = fallbackUser;
            }
            ...
            // Initialiser sans couper les données : plage complète
            filterTasksByPeriod(reportsWindow, 'all');
        } else {
            throw new Error(data.error || 'Erreur inconnue du serveur');
        }
    } catch (error) {
        console.error('Error loading report data:', error);
        console.error('Computed backendUrl for reports:', backendUrl);
        reportsDoc.getElementById('reportsContent').innerHTML = `
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
                ...
                <p class="mt-1 text-sm text-red-600">URL: ${backendUrl || 'inconnu'}/api/reports/data</p>
...
function filterTasksByPeriod(reportsWindow, period) {
    ...
    // Si aucun résultat, revenir au jeu complet pour éviter un tableau vide
    if (!filteredTasks.length && Array.isArray(reportsWindow.currentTasks)) {
        filteredTasks = reportsWindow.currentTasks;
    }
...
function generateReportsContent(reportsWindow, filteredTasks) {
    ...
    const user = getReportsUser(reportsWindow) || {};
    if (user.role === 'Department_Responsible') {
        ...
        contentDiv.innerHTML = generateEmployeeView(reportsWindow, filteredTasks, user);
    }
}
...
function getEmployeePersonalStats(tasks, user) {
    const me = user || {};
    const myTasks = tasks.filter(task => {
        if (!task.assignees || !Array.isArray(task.assignees)) return false;
        return task.assignees.some(assignee => 
            assignee.id === me.id ||
            `${assignee.first_name} ${assignee.last_name}` === `${me.first_name} ${me.last_name}`
        );
    });
    ...
    myTasks.forEach(task => {
        const myAssignee = task.assignees.find(a => 
            a.id === me.id || 
            `${a.first_name} ${a.last_name}` === `${me.first_name} ${me.last_name}`
        );
...
function generateEmployeeView(reportsWindow, filteredTasks, providedUser) {
    const user = providedUser || getReportsUser(reportsWindow) || {};
    const personalStats = getEmployeePersonalStats(filteredTasks, user);
    ...
    const accomplishedTasks = personalStats.tasks.filter(task => {
        const myAssignee = task.assignees.find(a => 
            a.id === user.id || 
            `${a.first_name} ${a.last_name}` === `${user.first_name} ${user.last_name}`
        );
